...
use libc::{AF_UNIX, c_char, sa_family_t, sockaddr, sockaddr_un, socklen_t};
...
impl Display for UnixSocketAddr {
    fn fmt(&self, fmtr: &mut fmt::Formatter) -> fmt::Result {
        match self.into() {
            ...
            UnixSocketAddrRef::Abstract(name) => {
                write!(fmtr, "@{}", OsStr::from_bytes(name).to_string_lossy())
            } // FIXME escape to sane characters
        }
    }
}
...
impl UnixSocketAddr {
    ...
    #[cfg_attr(
        not(any(target_vendor = "apple", target_os = "openbsd")),
        doc = "```no_run"
    )]
    /// # use std::os::unix::net::UnixDatagram;
    /// # use uds::{UnixSocketAddr, UnixDatagramExt};
