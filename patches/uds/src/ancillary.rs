...
use libc::{close, recvmsg, sendmsg};
use libc::{cmsghdr, iovec, msghdr, sockaddr, sockaddr_un};
...
use crate::credentials::{ReceivedCredentials, SendCredentials};
use crate::helpers::*;
    ...
    unsafe {
        ...
        if needed_capacity != 0 {
            ...
            #[cfg(not(any(target_os = "illumos", target_os = "solaris")))]
            {
                ...
                assert!(
                    !header_ptr.is_null(),
                    "CMSG_FIRSTHDR returned unexpected NULL pointer"
                );
                #[allow(unused_mut)]
                ...
                #[cfg(any(target_os = "linux", target_os = "android"))]
                {
                    if let Some(creds) = creds {
                        ...
                        *(CMSG_DATA(header) as *mut c_void as *mut _) = creds;
                        ...
                        assert!(
                            !header_ptr.is_null(),
                            "CMSG_NXTHDR returned unexpected NULL pointer"
                        );
...
impl AncillaryBuf {
        ...
        Self {
            ...
            ptr: match bytes {
                ...
                0..=Self::MAX_CAPACITY => unsafe {
                    let layout =
                        Layout::from_size_align(bytes as usize, mem::align_of::<cmsghdr>())
                            .unwrap();
    ...
    pub fn with_fd_capacity(num_fds: usize) -> Self {
        ...
        unsafe {
            ...
            let max_fds = (c_uint::max_value() - CMSG_SPACE(0)) as usize / mem::size_of::<RawFd>();
            if num_fds == 0 {
...
impl<'a> Iterator for FdSliceIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.slice.len > self.pos {
            ...
            let ret = unsafe { self.slice.unaligned_ptr.add(self.pos).read_unaligned() };
            self.pos += 1;
            Some(ret)
        } else {
            None
...
impl<'a> Iterator for Ancillary<'a> {
    ...
    fn next(&mut self) -> Option<AncillaryItem<'a>> {
        unsafe {
            ...
            let item = match (
                (*self.next_message).cmsg_level,
                (*self.next_message).cmsg_type,
            ) {
