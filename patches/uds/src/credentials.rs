...
use libc::{LOCAL_PEERCRED, XUCRED_VERSION, xucred};
#[cfg(target_os = "netbsd")]
use libc::{LOCAL_PEEREID, unpcbid};
#[cfg(any(target_os = "linux", target_os = "android"))]
use libc::{SO_PEERCRED, SO_PEERSEC, SOL_SOCKET, ucred};
#[cfg(target_os = "openbsd")]
use libc::{SO_PEERCRED, SOL_SOCKET, sockpeercred};
#[cfg(any(
    target_os = "linux",
    target_os = "android",
    target_os = "freebsd",
...
use libc::{c_void, getsockopt, socklen_t};
#[cfg(any(target_os = "linux", target_os = "android"))]
use libc::{getegid, geteuid, getgid, getpid, getuid, gid_t, pid_t, uid_t};
#[cfg(any(target_os = "illumos", target_os = "solaris"))]
use libc::{getpeerucred, ucred_free, ucred_t};
#[cfg(any(target_os = "illumos", target_os = "solaris"))]
use libc::{gid_t, pid_t, ucred_getegid, ucred_geteuid, ucred_getgroups, ucred_getpid, uid_t};
...
impl ConnCredentials {
    ...
    pub fn groups(&self) -> &[u32] {
        match self {
            ...
            &ConnCredentials::MacOsLike {
                number_of_groups: n @ 0..=15,
                ref groups,
                ..
            } => &groups[..(n as usize)],
            &ConnCredentials::MacOsLike {
                number_of_groups: 16..=255,
                ref groups,
                ..
            } => groups,
        }
...
pub fn peer_credentials(conn: RawFd) -> Result<ConnCredentials, io::Error> {
    ...
    unsafe {
        ...
        match getsockopt(conn, PEERCRED_SOCKET_LEVEL, LOCAL_PEERCRED, ptr, &mut size) {
            -1 => Err(io::Error::last_os_error()),
            _ if xucred.cr_version != XUCRED_VERSION => Err(io::Error::new(
                InvalidData,
                "unknown version of peer credentials",
            )),
            _ => {
