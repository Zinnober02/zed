...
use libc::{AF_UNIX, c_int, sockaddr, socklen_t};
use libc::{EINVAL, F_DUPFD_CLOEXEC, dup, fcntl};
#[cfg(not(any(
    target_vendor = "apple",
    target_os = "haiku",
    all(target_os = "android", target_arch = "x86")
)))]
use libc::{ENOSYS, accept4};
...
use libc::{accept, close, listen, socket, socketpair};
use libc::{bind, connect, getpeername, getsockname};
...
pub fn set_cloexec(fd: RawFd, close_on_exec: bool) -> Result<(), io::Error> {
    ...
    match cvt!(unsafe { ioctl(fd, op) }) {
        Ok(_) => Ok(()),
        #[cfg(any(target_os = "illumos", target_os = "solaris"))]
        Err(ref e) if e.kind() == ErrorKind::InvalidInput => unsafe {
            let prev = cvt!(fcntl(fd, F_GETFD))?;
            let change_to = if close_on_exec {
                prev | FD_CLOEXEC
            } else {
                prev & !FD_CLOEXEC
            };
...
pub fn get_unix_addr(socket: RawFd, get_side: GetAddr) -> Result<UnixSocketAddr, io::Error> {
    unsafe {
...
pub fn get_timeout(
    ...
    if timeout.tv_sec < 0 {
        ...
        Err(io::Error::new(
            ErrorKind::InvalidData,
            "timeout has invalid microsecond part",
        ))
...
impl Socket {
    ...
    pub fn new(socket_type: c_int, nonblocking: bool) -> Result<Self, io::Error> {
        ...
        {
            let type_flags =
                socket_type | SOCK_CLOEXEC | if nonblocking { SOCK_NONBLOCK } else { 0 };
    ...
    pub fn accept_from(fd: RawFd, nonblocking: bool) -> Result<(Self, UnixSocketAddr), io::Error> {
        unsafe {
            UnixSocketAddr::new_from_ffi(|addr_ptr, len_ptr| {
