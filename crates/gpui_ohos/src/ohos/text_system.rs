...
impl PlatformTextSystem for OhosTextSystem {
    ...
    fn font_id(&self, font: &Font) -> Result<FontId> {
        ...
        // Pick the candidate whose weight/style best matches the request.
        // (font-kit is not available on OHOS, so score against fontdb faces.)
        let want_weight = font.weight.0;
        let want_style = font.style;
        let mut index = 0usize;
        let mut best_score = i32::MAX;
        for (i, font_id) in candidates.iter().enumerate() {
            let database_id = state.loaded_font(*font_id).font.id();
            let score = state
                .font_system
                .db()
                .face(database_id)
                .map(|face| font_match_score(face, want_weight, want_style))
                .unwrap_or(i32::MAX);
            if score < best_score {
                best_score = score;
                index = i;
...
fn font_match_score(
    ...
    want_weight: f32,
    want_style: FontStyle,
) -> i32 {
    let style_penalty = match (want_style, face.style) {
    ...
    let weight_penalty = (face.weight.0 as f32 - want_weight).abs() as i32;
