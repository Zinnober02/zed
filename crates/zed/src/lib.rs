//! Zed's application startup, shared by the desktop binary and the OHOS
//! entry point so both run exactly the same initialization.
...
/// Run the application: parse arguments, initialize, and hand control to
/// GPUI's event loop.
pub fn run() {
